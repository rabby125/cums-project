...
if (empty($deviceId) || empty($targetUsername) || empty($newPassword)) {
    echo json_encode(["status" => "error", "message" => "Device, username and new password required"]);
    exit;
}
...
try {
    ...
    $logStmt = $pdo->prepare("INSERT INTO activity_logs (admin_id, username_target, device_id, action, result, details) VALUES (:admin_id, :username_target, :device_id, 'reset_password', :result, :details)");
    $logStmt->execute([
